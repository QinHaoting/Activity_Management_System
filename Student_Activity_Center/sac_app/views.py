	...
	if request.method == 'POST':
		...
		if con_code.upper() == check_code.upper():
			if method == 'stu':  # 如果选择学生登录
				try:
					student = students.objects.get(stu_id=log_id, stu_valid=1)
				...
				try:
					organizer = organizers.objects.get(org_id=log_id, org_valid=1)
...
def stu_activity_details(request, id):
	...
	stu = students.objects.filter(stu_id=stu_id).first()
	s = 1
	if stu.stu_name == None or stu.stu_phone == None:
		s = 0
	act = activities.objects.filter(act_id=id).first()
	print(act.id)
	team = teams.objects.filter(team_act_id=act.id).first()
	print(team)
	...
	if act in acts:
		...
		if act.act_type == 0:
			if ok == "1":  # 单人活动报名
				act_to_stu.objects.create(act_id=int(act.id), stu_id=stu.id)
...
def stu_center(request):
	...
	return render(request, 'stu_home/stu_center.html', context=context)
...
def stu_modify_message(request, id):
	...
	if request.method == "POST":
		...
		if stu_directMessages.objects.filter(id=de, message_valid=1).first():
			...
			msg.save()
		stu_valid = 1
		stu2.stu_name = request.POST.get('stu_name')
		stu2.stu_password = request.POST.get('stu_password')
		stu2.stu_phone = request.POST.get('stu_phone')
		stu2.stu_gender = request.POST.get('stu_gender')
		stu2.stu_grade = request.POST.get('stu_grade')
		stu2.stu_major = request.POST.get('stu_major')
		stu2.stu_Email = request.POST.get('stu_Email')
		stu2.stu_introduction = request.POST.get('stu_introduction')
		stu2.stu_college = request.POST.get("stu_college")
		stu2.valid = stu_valid
		stu2.save()
		return redirect(reverse('sac_app:stu_center'), locals())
	return render(request, 'stu_home/stu_modify_message.html', locals())
...
def stu_activity(request):
	...
	for a in whole_activities:
		if a.act_state == 2:
			a.act_flag = "可参加"
			a.save()
	print(whole_activities)
	...
	if show_activities:  # 待显示的活动不为空
		...
		return HttpResponse(render(request, 'stu_home/stu_activity.html', context=context))  # 访问成功
	else:  # 待显示列表为空
		context = {
		...
		return HttpResponse(render(request, 'stu_home/stu_activity.html', context=context))
...
def stu_join_activity_details(request, id):
	...
	return render(request, 'stu_home/stu_join_activity_details.html', context=context)
...
def stu_create_team(request):
	...
	count = len(messages)
	if request.method == "GET":
		return render(request, 'stu_home/stu_create_team.html', locals())
	else:
		de = request.POST.get('delete')
		if stu_directMessages.objects.filter(id=de, message_valid=1).first():
			msg = stu_directMessages.objects.filter(id=de, message_valid=1).first()
		...
		new_team = teams.objects.create(team_number=team_number,
										...
										team_act=act)
		act.act_available_team_number = act.act_available_team_number - 1
		act.save()
		# 学生与活动表间关系
...
def stu_myteam(request):
	...
	if stu:
		try:
			...
			page1 = pgnt1.page(page)
			page2 = list(page1)
			return HttpResponse(render(request, 'stu_home/stu_myteam.html', locals()))
		except EmptyPage:
...
def stu_view_myteam(request, id):
	team = teams.objects.filter(id=int(id)).first()
	stus_id = stu_to_team.objects.filter(team_id=team.id).values()
	print(stus_id)
	stus = []
	for i in stus_id:
		stus.append(students.objects.get(id=i["stu_id"]))
	context = {
		'team': team,
		'control': 1,
		"stus": stus
...
def act_join_other_team(request, id):
	...
	if act:
		try:
			...
			print(team)
			pgnt1 = Paginator(team, 10)
			page = request.POST.get('page', 1)
			ok = request.POST.get('ok')
			page2 = list(pgnt1.page(page))
			return render(request, 'stu_home/stu_join_other_team.html', locals())
		except EmptyPage:
			return HttpResponse(render(request, 'stu_home/stu_join_other_team.html', locals()))
	return HttpResponse(render(request, 'stu_home/stu_join_other_team.html', locals()))
...
def stu_view_other_team(request, id):
	...
	if request.method == "POST":
		...
		stu_id = request.session.get('user_id')
		stu = students.objects.get(stu_id=stu_id)
		stu_to_team.objects.create(stu_id=stu.id, team_id=id)
		act_to_stu.objects.create(stu_id=stu.id, act_id=team.team_act_id)
		return HttpResponse(render(request, 'stu_home/stu_view_other_team.html'), locals())
	else:
		return render(request, 'stu_home/stu_view_other_team.html', locals())
	...
	if request.method == 'GET':
		...
		if request.method == "POST":
			...
			if org_directMessages.objects.filter(id=de, message_valid=1).first():
				...
				msg.message_valid = 0
				msg.save()
		return render(request, 'org_home/org_center.html', locals())
...
def org_launch_activity(request):  # 需要修改
	...
	id = request.session.get('user_id')
	print('idid ', id)
	org = organizers.objects.filter(org_id=id, org_valid=1).first()
	accept_id = request.session.get('user_id', 1)
	print('org:****:', org.org_name)
	print(accept_id)
	messages = list(org_directMessages.objects.filter(accept_id=accept_id, message_valid=1))
	count = len(messages)
	if request.method == 'GET':
		return render(request, 'org_home/org_launch_activity.html', locals())
		...
		ac = activities.objects.create(
			...
			act_organizer_name=org.org_header_name,
			act_organizer=org,
...
def org_launch_notice(request):
	...
	if request.method == 'POST':
		...
		notices.objects.create(
			...
			notice_appendix=attachment,
			notice_tag=1
		)
		success = "发布成功"
		return render(request, 'org_home/org_launch_notice.html', locals())
...
def org_modify_activity_message(request, id):
	org_id = request.session.get('user_id')
	org = organizers.objects.get(org_id=org_id, org_valid=1)
	act = activities.objects.get(act_id=id)
	accept_id = request.session.get('user_id', 1)

	messages = list(org_directMessages.objects.filter(accept_id=accept_id, message_valid=1))
	count = len(messages)
	print("zuzhizhe", org.org_id)
	print(id)
	if request.method == 'POST':
		de = request.POST.get('delete')
		if org_directMessages.objects.filter(id=de, message_valid=1).first():
		...
		act.act_state = 0
		act.save()
		return redirect(reverse('sac_app:org_view_posted_activity'), locals())
	return render(request, 'org_home/org_modify_activity_message.html', locals())
...
def org_stu_one_list(request, id):
	...
	act = activities.objects.get(act_id=id)
	try:
		temp = list(act_to_stu.objects.filter(act_id=act.id).values('stu_id'))
		stu = []
		for i in range(len(temp)):
			stu.append(students.objects.get(id=temp[i]['stu_id']))
		pgnt1 = Paginator(stu, 10)
		pagenum = request.POST.get('page', 1)
		page = list(pgnt1.page(pagenum))
		return HttpResponse(render(request, 'org_home/org_stu_one_list.html', locals()))
	except EmptyPage:
...
def org_stu_team_list(request, id):
	...
	try:
		...
		pagenum = request.POST.get('page', 1)
		page = list(pgnt1.page(pagenum))
		return HttpResponse(render(request, 'org_home/org_stu_team_list.html', locals()))
	except EmptyPage:
...
def org_stu_team_list_mul(request, id):
	...
	m = list(stu_to_team.objects.filter(team_id=stu_team.id).values('stu_id'))
	stus = []
	for i in range(len(m)):
		stus.append(students.objects.get(id=m[i]['stu_id']))
	return render(request, 'org_home/org_stu_team_list_mul.html', locals())
...
def mag_examine_act(request):
	...
	try:
		...
		# page = request.POST.get('page',1)
		qs1 = []
		qs = activities.objects.filter(act_state=0)
		for q in qs:
			if activities_modified.objects.filter(act_valid=1, act_state=0, act_id=q.act_id).first():
				c = 1  # .values().order_by('org_id')#'notice_title')
			else:
				qs1.append(q)
		qs2 = activities_modified.objects.filter(act_valid=1, act_state=0)
		print(qs1)
		print(qs2)
		pgnt1 = Paginator(qs1, pagesize)
...
def mag_look_act(request, id):
	...
	"""
	act = activities.objects.filter(act_id=id).first()
	act_mid = activities_modified.objects.filter(act_valid=1, act_id=id).first()

	if request.method == 'POST':
		sel = request.POST.get('select')
		msg = request.POST.get('msg')

		org_id = act.act_organizer.org_id
		if sel == 'yes':
			if act_mid:
			...
			message = '您拟举办的活动' + act.act_name + '审核通过。' + ' 通过理由:' + msg
			org_directMessages.objects.create(accept_id=org_id,
											  message=message,
											  message_valid=1)
			...
			if act_mid:
				...
				act.save()
			org_id = act.act_organizer.org_id
			message = '很遗憾的通知您，您拟举办的活动' + act.act_name + '审核不通过。' + ' 不通过理由:' + msg
			org_directMessages.objects.create(accept_id=org_id,
											  message=message,
											  message_valid=1)
			return redirect(reverse('sac_app:mag_examine_act'))
...
def mag_look_org(request, id):
	...
	if request.method == 'POST':
		...
		msg = request.POST.get('msg')
		if sel == 'yes':
			message = '您拟修改的社团信息，审核成功。' + ' 通过理由:' + msg
			org_directMessages.objects.create(accept_id=org.org_id,
											  message=message,
											  message_valid=1
											  )
			...
			message = '很遗憾的通知你，您拟修改的社团信息，审核不通过。' + ' 不通过理由:' + msg
			org_directMessages.objects.create(accept_id=org.org_id,
											  message=message,
											  message_valid=1
											  )
...
def mag_revise(request, id):
	org = organizers.objects.filter(org_id=id, org_valid=1).first()
	organizer = organizers.objects.filter(org_id=id, org_valid=1).first()
	if request.method == 'POST':
...
def stu_bbs(request):
	...
	if request.method == 'POST':
		...
		bbs_comments.objects.create(bbs_id=id, bbs_message=com)
		page = request.GET.get('page', 1)
		comment = bbs_comments.objects.all().order_by('bbs_create_time')
		pgnt = Paginator(comment, 10)
		comlist = list(pgnt.page(page))
